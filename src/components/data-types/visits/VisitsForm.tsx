...
import { createVisit, updateVisit } from "@/tableInteractions/actions";

export default function VisitsForm({
	visit,
	onSuccess,
}: {
	visit?: z.infer<typeof generalSchema> & { id: string };
	onSuccess?: () => void;
}) {
	...
	const onSubmit = async (values: z.infer<typeof generalSchema>) => {
		const action = visit == null ? createVisit : updateVisit.bind(null, visit.id);
