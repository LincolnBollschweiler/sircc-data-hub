...
import { createReferredOut, updateReferredOut } from "@/tableInteractions/actions";

export default function ReferredOutForm({
	referredOut,
	onSuccess,
}: {
	referredOut?: z.infer<typeof generalSchema> & { id: string };
	onSuccess?: () => void;
}) {
	const form = useForm<z.infer<typeof generalSchema>>({
		resolver: zodResolver(generalSchema),
		defaultValues: referredOut ?? {
			name: "",
			description: "",
		},
	});
	...
	const onSubmit = async (values: z.infer<typeof generalSchema>) => {
		const action = referredOut == null ? createReferredOut : updateReferredOut.bind(null, referredOut.id);
